...
#ifdef ISO_TP_DEBUG
    printf("C Debug: isotp_send_with_id - Attempting to send on arbitration ID: 0x%X\n", (unsigned int)id);
    printf("C Debug: isotp_send_with_id - Payload size: %d, First byte: 0x%02X\n", size, payload[0]);
#endif
    int ret;
...
#ifdef ISO_TP_DEBUG
    printf("C Debug: isotp_send_with_id - ISO-TP send attempt finished with status: %d\n", ret);
#endif
    return ret;
}
